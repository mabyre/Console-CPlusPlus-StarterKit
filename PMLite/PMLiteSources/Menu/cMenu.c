...
{
   PMMenuLine* self = (PMMenuLine*)pm_malloc( sizeof(PMMenuLine) );
   
   if (self != 0)
      pm_memset( self, 0, sizeof(PMMenuLine) );
...
{
   PMMenu*  self = (PMMenu*)pm_malloc( sizeof( PMMenu ) );
   
   if (self != 0)
   {
      pm_memset(self, 0, sizeof(PMMenu));
      self->itsProc = aProc;
   }
...
pmbool PMMenu_AddLineToMenu
( 
    PMMenu* self, 
    pmuint16 aCode, 
    const char *aText, 
    PMMenu* aSubMenu, 
    PMMenuItemProc aProc
)
{
   PMMenuLine* theLine, *theLastLine;

   if ((aSubMenu != 0) && (aProc != 0))
      PM_ERROR0(TL("PMMenu_AddLineToMenu(): Both aMenu and aProc are not 0 ???"));
   ...
   if (aCode != kPMMenuSeparator)
   {
      ...
      while (theLine != 0)
      {
         ...
         {
            PM_ERROR0(TL("PMMenu_AddLineToMenu(): a line already exists with code (%lu)", (unsigned long)aCode));
            return pmfalse;
         }
...
void PMMenu_Execute(PMMenu* self, pmint32 aLevel)
{
   ...
   {
      PM_ERROR0(TL("PMMenu_Execute(): Empty menu."));
      return;
   }
