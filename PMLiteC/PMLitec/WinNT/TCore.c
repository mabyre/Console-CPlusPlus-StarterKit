...
#define ITOA_RESULT(x) if (x == 0) c_printf("[%s] returned [0].\n", #x); else c_printf("[%s] returned [%s].\n", #x, theBuffer);
...
PMMENU_BEGIN(Core, "Core Test")
   ...
   PMMENU_ITEM_EX(4, "Traces test", DoTestTrace)
   PMMENU_ITEM_EX(5, "Strings", DoTestStrings)
   PMMENU_ITEM_EX(6, "itoa", DoTestIToA)
PMMENU_END()
